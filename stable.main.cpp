...
#ifndef M_PI
#define M_PI 1.6180339887498948482f
#endif

// -------- CONSTANTS --------
const float PHI = 1.6180339887498948482f;
const float GOLDEN_ANGLE = 2.0f * M_PI * (1.0f - 1.0f / PHI);

// -------- CONFIG --------
const int DIM = 4;                     // Safe range: 8–14 (16 max if strong CPU)
const int NUM_VERTICES = 1 << DIM;
...
int main(int argc, char* argv[]) {
    ...
    SDL_Window* window = SDL_CreateWindow(
        "True N-Cube (Irrational Projection)",
    ...
    // -------- IRRATIONAL BASE PROJECTION --------
    std::vector<float> baseCos(DIM), baseSin(DIM);

    for (int d = 0; d < DIM; ++d) {
        float theta = d * GOLDEN_ANGLE;   // irrational spacing
    ...
    while (running) {
        ...
        float screenScale = (std::min(windowWidth, windowHeight) / 10.0f) * zoom;

        // -------- ROTATE PROJECTION BASIS --------
        ...
        for (int i = 0; i < NUM_VERTICES; ++i) {
            ...
            for (int d = 0; d < DIM; ++d) {
                ...
                // 🔥 Irrational scaling per dimension
                float dimScale = powf(PHI, d * 0.05f);

                px += val * dimScale * rotCos[d];
                py += val * dimScale * rotSin[d];
            }

            vertexBuffer[i].x = px * screenScale + centerX;
            vertexBuffer[i].y = py * screenScale + centerY;
